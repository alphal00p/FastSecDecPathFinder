...
One-loop massless box from the DOT graph, integrated with FSD's QMC route for
about 30 seconds and compared against the compatible OneLOop target from the
run preset:
...
The same one-loop box, but using pySecDec as the native package-generation and
integration engine.  FSD only prepares the DOT/kinematics boundary and calls
pySecDec; the generated pySecDec package is kept under `MyPySecDecOutput`:
...
Massless two-loop double box from the DOT graph, integrated with FSD/Havana and
compared against the stored pySecDec-convention target:
...
The same double-box integral supplied directly as Symanzik `U` and `F`
polynomials.  The run preset also supplies the required parametric metadata,
including propagator powers, U/F epsilon exponents, and global prefactor:
